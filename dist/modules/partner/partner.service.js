...
let PartnerService = class PartnerService {
    ...
    async getDashboard(partnerId) {
        const [[clients], [employees], [expenses]] = await Promise.all([
            this.dataSource.query(`SELECT COUNT(*)::int AS total FROM clients WHERE partner_id = $1`, [partnerId]),
            this.dataSource.query(`SELECT COUNT(e.id)::int AS total FROM employees e
        ...
        return {
            ...
            totalAmountVnd: expenses.total_amount_vnd,
            expensesThisMonth: expenses.this_month,
        };
    }
    async listClients(partnerId) {
        const rows = await this.dataSource.query(`SELECT c.id, c.name, c.is_active, c.created_at,
    ...
    async createClient(partnerId, dto) {
        const existing = await this.dataSource.query(`SELECT id FROM clients WHERE partner_id = $1 AND name = $2 LIMIT 1`, [partnerId, dto.name]);
        if (existing.length)
            throw new common_1.ConflictException('Client with this name already exists');
        const rows = await this.dataSource.query(`INSERT INTO clients (partner_id, name) VALUES ($1, $2) RETURNING id, name, is_active, created_at`, [partnerId, dto.name]);
        const r = rows[0];
    ...
    async updateClient(clientId, partnerId, dto) {
        ...
        if (!setClauses.length)
            return this.getClient(clientId, partnerId);
        params.push(clientId, partnerId);
        const updated = await this.dataSource.query(`UPDATE clients SET ${setClauses.join(', ')} WHERE id = $${i} AND partner_id = $${i + 1} RETURNING id`, params);
        if (!updated.length)
            throw new common_1.NotFoundException('Client not found');
        return this.getClient(clientId, partnerId);
    }
    async getClient(clientId, partnerId) {
        const rows = await this.dataSource.query(`SELECT c.id, c.name, c.is_active, c.created_at,
              COUNT(DISTINCT e.id)::int  AS employee_count,
              COUNT(DISTINCT ex.id)::int AS expense_count,
              COUNT(DISTINCT ex.id) FILTER (WHERE ex.status = 'pending')::int AS pending_count
    ...
    async listEmployees(partnerId, clientId) {
        ...
        const rows = await this.dataSource.query(`SELECT e.id, e.employee_id, e.full_name, e.is_active, e.pdpd_consent,
    ...
    async updateEmployee(employeeId, partnerId, dto) {
        ...
        const updated = await this.dataSource.query(`UPDATE employees e SET ${setClauses.join(', ')}
       FROM clients c
    ...
    async getReports(partnerId, query) {
        ...
        const rows = await this.dataSource.query(`SELECT c.id AS client_id, c.name AS client_name,
